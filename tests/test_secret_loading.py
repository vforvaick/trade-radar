...
def test_telegram_credentials_use_env_and_cli_overrides(module, monkeypatch):
    monkeypatch.setenv("PUMPRADAR_TG_TOKEN", "env-token")
    monkeypatch.setenv("PUMPRADAR_TG_CHAT", "env-chat")
...
def test_telegram_credentials_can_disable_telegram_when_both_missing(module, monkeypatch):
    monkeypatch.delenv("PUMPRADAR_TG_TOKEN", raising=False)
    monkeypatch.delenv("PUMPRADAR_TG_CHAT", raising=False)
...
def test_telegram_credentials_fail_fast_when_partial(module, monkeypatch):
    monkeypatch.setenv("PUMPRADAR_TG_TOKEN", "env-token")
    monkeypatch.setenv("PUMPRADAR_TG_CHAT", "   ")
...
def test_telethon_settings_use_env(module, monkeypatch):
    monkeypatch.setenv("PUMPRADAR_TG_API_ID", "123456")
    monkeypatch.setenv("PUMPRADAR_TG_API_HASH", "hash-value")
    monkeypatch.setenv("PUMPRADAR_TG_SESSION", "session-name")
...
def test_telethon_settings_fail_fast_on_missing_or_invalid(module, monkeypatch):
    monkeypatch.setenv("PUMPRADAR_TG_API_ID", "   ")
    monkeypatch.setenv("PUMPRADAR_TG_API_HASH", "")
    monkeypatch.delenv("PUMPRADAR_TG_SESSION", raising=False)

    with pytest.raises(SystemExit, match="Missing Telethon credentials"):
        module.load_telethon_settings()

    monkeypatch.setenv("PUMPRADAR_TG_API_ID", "not-an-integer")
    monkeypatch.setenv("PUMPRADAR_TG_API_HASH", "hash-value")
    monkeypatch.setenv("PUMPRADAR_TG_SESSION", "session-name")

    with pytest.raises(SystemExit, match="PUMPRADAR_TG_API_ID must be a valid integer"):
        module.load_telethon_settings()
